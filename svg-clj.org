...
(def sk-url "/Users/adam/dev/forge/sk.svg")
(def sk (slurp sk-url))
(def sk-elems (tools/load-svg sk-url))
(def polygon-with-curves
  (->> sk-elems
       (drop 2) last
       (drop 2) last))

(tools/cider-show polygon-with-curves)
...
(def poly-elems
  (->> polygon-with-curves
       tf/path->elements
       (map rand-stroke)
       (map #(tf/style % {:fill "none"}))))

(tools/cider-show poly-elems)

(def path-elems (map tf/element->path poly-elems))

(tools/cider-show path-elems)

(def path-elem (tf/elements->path poly-elems))

(tools/cider-show path-elem)

(def closed-path-elem (tf/merge-paths path-elem [:path {:d "Z"}]))

(def merged-poly
  (apply tf/merge-paths path-elems))
