...
*** GreenCoder
 ...
 - use str/join instead of the interpose " " pattern where possible. 
 #+begin_src clojure
 ;; rewrite polygon
 ...
 (defn polygon
   [pts]
   [:polygon {:points (str/join " " (mapv utils/v->s pts))}])


 #+end_src

 - :refer :all is deprecated 
...
If I run translate on a list of elements, it incorrectly nests everything, so I get N levels of depth, where N is number of elements in the list. It's just wrong.

But, if I map translate-element over the list, it works correctly.

Basically, i think I should remove the need for the 'interface' fn and just have users directly use the translate-elemtn, rotate-elem, etc. This should also eliminate the problem where I'm using spec in conditional branching directly.

#+begin_src clojure
;; very wrong
(comment 
(tf/rotate 45 b)
([:circle {:cx 0, :cy 0, :r 2, :transform "rotate(45 0 0)"}]
 ([:circle {:cx 0, :cy 0, :r 2, :transform "rotate(45 0 0)"}]
  ([:circle {:cx 0, :cy 0, :r 2, :transform "rotate(45 0 0)"}]
   ([:circle {:cx 0, :cy 0, :r 2, :transform "rotate(45 0 0)"}]
    ([:circle {:cx 0, :cy 0, :r 2, :transform "rotate(45 0 0)"}]
...
;; correct output
(comment
(map #(tf/translate [10 10] %) b)
([:circle {:cx 10, :cy 10, :r 2, :transform "rotate(0 10 10)"}]
 [:circle {:cx 10, :cy 10, :r 2, :transform "rotate(0 10 10)"}]
 [:circle {:cx 10, :cy 10, :r 2, :transform "rotate(0 10 10)"}]
 [:circle {:cx 10, :cy 10, :r 2, :transform "rotate(0 10 10)"}]
 [:circle {:cx 10, :cy 10, :r 2, :transform "rotate(0 10 10)"}]
...
*** interface
see if I can re-work the translate-element fn to eliminate the need for this interface. 
...
svg-clj.main/->edn <f>
svg-clj.main/circle <f>
svg-clj.main/ellipse <f>
svg-clj.main/g <f>
svg-clj.main/image <f>
svg-clj.main/line <f>
svg-clj.main/polygon <f>
svg-clj.main/polyline <f>
svg-clj.main/rect <f>
svg-clj.main/style <f>
svg-clj.main/svg <f> 	svg-clj.main/text <f>
svg-clj.main/unwrap-elements <f>
svg-clj.main/xml->hiccup <f>
