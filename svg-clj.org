...
This is manually added to reflect-config.json to replace what the agent finds. Don't know why, but the one that the agent found on my attempts did not work, but this one (taken from clj-graal-docs github repo) does let the image compile.
...
{:tasks
 {:requires ([babashka.fs :as fs]
  ...
  gen-reflect
  {:doc "Generate reflect-config.json by running uberjar with agent."
   :depends [uberjar]
   :task (apply shell "java -agentlib:native-image-agent=caller-filter-file=filter.json,config-output-dir=. -jar svg_clj.jar" *command-line-args*)}
